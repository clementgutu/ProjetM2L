...
$navLinkHref = '../index.php';
$navLinkText = "S'inscrire";
$navLinkIcon = 'fa-user-plus';
...
.alert-in    { animation: alertSlide 0.3s ease both; }
.success-in  { animation: successPop 0.4s cubic-bezier(.22,.68,0,1.2) both; }
...
<div class="min-h-screen flex">
    ...
    <div class="anim-panel hidden lg:flex lg:w-5/12 bg-gradient-to-br from-red-900 via-red-700 to-red-500 flex-col items-center justify-center p-12 relative overflow-hidden">
        ...
        <div class="relative z-10 text-center">
            ...
            <p class="text-red-100 text-sm mb-10 leading-relaxed">
                Votre espace collaboratif interne.<br>Retrouvez tous vos collègues facilement.
            </p>
            <div class="space-y-3">
    ...
    <!-- ── Right panel ────────────────────────── -->
    <div class="w-full lg:w-7/12 flex items-center justify-center p-6 pt-24 lg:pt-6 bg-gray-50"
         x-data="{
             showPwd: false,
             loading: false,
             submitForm(e) {
        ...
        <div class="anim-card w-full max-w-md">
            ...
            <!-- Title -->
            <div class="mb-8">
                <h2 class="text-3xl font-bold text-gray-900 mb-1">Bienvenue !</h2>
                <p class="text-gray-400 text-sm">Connectez-vous à votre espace de travail.</p>
            </div>

            <!-- ── Alerts ─────────────────────── -->
            <?php if (!empty($erreur_connexion)): ?>
            <div class="alert-in shake flex items-start gap-3 bg-red-50 border border-red-200 text-red-700 px-4 py-3.5 rounded-xl text-sm mb-6">
                <i class="fas fa-circle-exclamation flex-shrink-0 mt-0.5"></i>
                <span><?= htmlspecialchars($erreur_connexion) ?></span>
            </div>
            <?php endif; ?>

            <?php if (isset($_GET['deconnecte']) && $_GET['deconnecte'] == 1): ?>
            <div class="alert-in flex items-start gap-3 bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3.5 rounded-xl text-sm mb-6">
                <i class="fas fa-circle-check flex-shrink-0 mt-0.5"></i>
                <span>Vous avez été déconnecté avec succès.</span>
            </div>
            <?php endif; ?>
            <?php if (isset($_GET['registered']) && $_GET['registered'] == 1): ?>
            <div class="success-in flex items-start gap-3 bg-green-50 border border-green-200 text-green-700 px-4 py-3.5 rounded-xl text-sm mb-6">
                <i class="fas fa-circle-check flex-shrink-0 mt-0.5 text-green-500"></i>
                <span>Inscription réussie ! Vous pouvez maintenant vous connecter.</span>
            </div>
            <?php endif; ?>

            <!-- ── Form ───────────────────────── -->
            <form action="" method="POST" class="space-y-5" @submit.prevent="submitForm($event)">
                ...
                <!-- Mot de passe -->
                <div>
                    <label for="motdepasse" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                        Mot de passe
                    </label>
                    <div class="inp-wrap relative">
                        <i class="inp-icon fas fa-lock"></i>
                        <input :type="showPwd ? 'text' : 'password'" id="motdepasse" name="motdepasse"
                               placeholder="••••••••"
                               class="inp pr-11"
            ...
            <p class="text-center text-gray-500 text-sm">
                ...
                <a href="../index.php" class="text-red-600 font-semibold hover:text-red-700 transition-colors hover:underline">
                    Créer un compte
                </a>
            </p>
